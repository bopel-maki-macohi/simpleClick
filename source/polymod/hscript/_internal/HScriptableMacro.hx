				...
				switch field.kind
				{
					case FFun(func):
						...
						if (polymod.util.DefineUtil.getDefineBoolRaw('POLYMOD_USE_NAMESPACE'))
						{
							...
							if (hscriptParams.id != null)
								pathName = '$module/${hscriptParams.id}/$pathName';
							else
								...
								try
								{
									...
									if (script == null)
									{
										if ($v{!hscriptOptional})
										{
											...
											polymod.Polymod.error(SCRIPT_NOT_FOUND, 'The scripted function ' + $v{pathName} + ' could not be found.',
												SCRIPT_RUNTIME);
										...
										{
											polymod.Polymod.debug('The scripted function '
		...
		for (paramField in paramFields)
		{
			switch (paramField.field)
			{
				...
				case 'id':
					...
					{
						case EConst(CString(value)):
							// Passed a string, this means id is constant.
							result.id = value;
						case EConst(CIdent(value)):
							// Passed an identifier, this means id is dynamic.
							result.idDynId = value;
						default:
							throw '@:hscript({id}) must be a String value.';
					}
				default:
