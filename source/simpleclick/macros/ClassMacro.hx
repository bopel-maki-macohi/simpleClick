...
class ClassMacro
{
	 ...
	 *
	 * @param targetPackageA String containing the Packagename to query.
	...
	public static macro function listClassesInPackage(targetPackage:String, includeSubPackages:Bool = true):ExprOf<Iterable<Class<Dynamic>>>
	{
		...
		return macro simpleclick.macros.CompiledClassList.get($v{request});
	}
	...
	public static macro function listSubclassesOf<T>(targetClassExpr:ExprOf<Class<T>>):ExprOf<List<Class<T>>>
	{
		...
		var targetClass:ClassType = simpleclick.macros.MacroUtil.getClassTypeFromExpr(targetClassExpr);
		...
		return macro simpleclick.macros.CompiledClassList.getTyped($v{request}, ${targetClassExpr});
	}
	...
	{
		var compiledClassList:ClassType = simpleclick.macros.MacroUtil.getClassType("simpleclick.macros.CompiledClassList");
	...
	static function doesClassMatchRequest(classType:ClassType, request:String):Bool
	{
		...
		switch (requestType)
		{
			...
			case 'extend':
				...
				var targetClassType:ClassType = simpleclick.macros.MacroUtil.getClassType(targetClassName);

				if (simpleclick.macros.MacroUtil.implementsInterface(classType, targetClassType))
				{
					return true;
				}
				else if (simpleclick.macros.MacroUtil.isSubclassOf(classType, targetClassType))
				{
					return true;
				}
