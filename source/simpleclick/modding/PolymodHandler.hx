...
import simpleclick.macros.ClassMacro;
...
class PolymodHandler
{
	...
	static function buildImports():Void
	{
		...
		static final DEFAULT_IMPORTS:Array<Class<Dynamic>> = [flixel.FlxG, simpleclick.modding.sandboxed.AssetsSandboxed];
		...
		Polymod.addImportAlias('lime.utils.Assets', simpleclick.modding.sandboxed.AssetsSandboxed);
		Polymod.addImportAlias('openfl.utils.Assets', simpleclick.modding.sandboxed.AssetsSandboxed);
		...
		// `Reflect`
		// Reflect.callMethod() can access blacklisted package simpleclick.s, but some functions are whitelisted
		Polymod.blacklistStaticFields(Reflect, ['callMethod']);

		// `Type`
		// Type.createInstance(Type.resolveClass()) can access blacklisted package simpleclick.s, but some functions are whitelisted
		...
		// `haxe.Unserializer`
		// Unserializer.DEFAULT_RESOLVER.resolveClass() can access blacklisted package simpleclick.s
		Polymod.blacklistImport('haxe.Unserializer');

		// `lime.utils.AssetLibrary`
		// If you create your own library using a manifest, AssetLibrary.__fromManifest() can access blacklisted package simpleclick.s apparently.
		...
		// `flixel.util.FlxSave`
		// resolveFlixelClasses() can access blacklisted package simpleclick.s
		...
		for (cls in ClassMacro.listClassesInPackage('simpleclick.macros'))
		{
			if (cls == null) continue;
			var className:String = Type.getClassName(cls);
			Polymod.blacklistImport(className);
